...
$(function (){
	manager=  $("#maingrid").ligerGrid({
		columns: [
		          ...
		          { display: '字段名称', name: 'field_name', align: 'center',
		        	  editor: { type: 'text' }
		          },
		          ...
		          { display: '操作', align: 'center' ,render: function (rowdata, rowindex, value)
	                  {
	                	  id = rowdata.field_id;
	                	  var h = "<div style='height:25px;'><a href=\"javascript:update('"+id+"')\" style=\"line-height:0;\"><image src=\"../../image/edit.png\">&nbsp;编辑</a></div>";
	                	  return h;
			          }
	                }
		         ],
		          isScroll: true,
		          url : '/epmwxwl/entityField/listAjax',
		          parms:{
		        	  "entity_id":entityId
		          },
		          enabledEdit: true, 
...
$("#search").click(function() {
	...
	var field_type_id = $("#field_type").val();
...
$("#movetd").click(function() {
	...
	if(nodes.length==0||nodes[0]==undefined||nodes[0]==null||nodes[0].isEntityField==0||nodes[0].isEntityField==2){
		...
		$.ajax({
			url: '../../entityField/updateAjax',
			type: "POST",
			async: true,
			data: 
			{
				"field_ids":str,
				"field_type_id":nodes[0].id,
				"entityId":entityId
			},
...
$(function(){
	typeTreeSetting = {
			...
			async: {
				...
				type:"POST",
				autoParam: ["id"],
				otherParam: { "entityId":entityId},
				url:"../../entityField/entityFiledType",
				dataFilter: null
			},
			callback: {
...
function updateTreeNode() {
	...
	if (zTree.getSelectedNodes()[0].id!="root") {
		...
		typedilog = $.ligerDialog.open({
			title:'编辑',
			url : '../../entityField/updateTypeJsp2/'+typeid,
			height : 200,
			isResize : true,
		});
	} else {
	...
	if (nodes && nodes.length>0) {
		if("root"==nodes[0].id){
			...
			//删除节点
				var typeid= nodes[0].id;
				$.ajax({
					url: '../../entityField/deleteFieldType',
					type: "POST",
					async: true,
					data: 
					{
...
function f_onAfterEdit(e){
	var entity_id = e.record.entity_id;
	var filed_id = e.record.field_id;
	var field_name = e.record.field_name;
	$.ajax({
		url: '../../entityField/updateNameById',
		type: "POST",
		async: true,
		data: 
		{	
