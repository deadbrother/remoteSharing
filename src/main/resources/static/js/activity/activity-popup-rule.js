		...
		columns : [           
		{
			...
			render:function(rowdata,index,value){
				if(rowdata.activityTheme==undefined||rowdata.activityTheme==""||rowdata.activityTheme==null){
					return "<div class='l-grid-row-cell-inner' style='max-height:20px; text-align:center;' ></div>";
				}else{
					return "<div class='l-grid-row-cell-inner' style='max-height:20px; text-align:center;' title='"+rowdata.activityTheme+"'>" +rowdata.activityTheme+ "</div>";
				}
...
function movePageAsyn(pre,after){
	...
	var themeCode = after ==null?manager.getRow(pre).themeCode:manager.getRow(after).themeCode;
	var order = after==null?"0":"1";
	$.ajax({
		url: '../activity/priorityOrdPageChange',
		type: "POST",
		async: true,
		data: {	
			"priorityOrdList":[{"activityId":activityId,"ord":ord,"orgLevel":orgLevel,"themeCode":themeCode},
			                   {"activityId":"","ord":order,"orgLevel":orgLevel,"themeCode":themeCode}]
		},
...
function moveAllAsyn(pre){
	$.ajax({
		url: '../activity/priorityOrdUpAllChange',
		type: "POST",
		async: true,
		data: 
		{	
			"priorityOrdList":[{"activityId":manager.getRow(pre).activityId,"ord":"1","orgLevel":manager.getRow(pre).activityLevel,"themeCode":manager.getRow(pre).themeCode},
			                {"activityId":"","ord":manager.getRow(pre).ord,"orgLevel":manager.getRow(pre).activityLevel,"themeCode":manager.getRow(pre).themeCode}]
		},
		success: function(data) {	
			manager.reload(1);
		}
...
function moveAsyn(pre,after){
	//console.log(pre);
	$.ajax({
		url: '../activity/priorityOrdChange',
		type: "POST",
		async: true,
		data: {	
