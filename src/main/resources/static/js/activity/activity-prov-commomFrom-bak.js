...
/**
 * 短信天发送时间时间段(messageSendTime)的回显逻辑
 */
...
function checkMax(){
	...
	$("[check_max]").each(function(){
		...
		if(parseInt(max)<parseInt(objVal)){
			$.ligerDialog.warn(checkName + "的大小不能超过" + min);
			return false;
		}
...
function isChecked(){
    ...
    if($("#orderEndTime").is(":checked")==true){
    	$("input[name='po.orderCycle']").val("-1");
    	//$("input[name='bo.po.orderCycle']").attr("readonly","readonly");
    	$("input[name='po.orderCycle']").attr("readonly",true);
    }else{
    	$("input[name='po.orderCycle']").attr("readonly",false);
    }
}
 ...
 function checkNum() {
	...
	$("[numcheck]").each(function() {
		...
		if (parseInt(objVal)=="NaN") {
			...
			$.ligerDialog.warn(check_name + "的值必须在0-100之间，请重新填写");
			return false;
		}
		flag=true;
	});
			...
			bpmServer.getDaibanTasks(loginId,
					...
					function(json){
						...
						$(json.data).each(function(){
							if(this.TASK_DESC == $(":input[name='po.activityId']").val()){
								flag = true;
						...
						if( flag == false){
							 ...
							 */
							var params = {};
							params.activityName = $(":input[name='po.activityName']").val().trim();
							var activityThemeCode = $(":input[name='po.activityThemeCode']").val();
							var activityTheme = "";
							$(":input[name='po.activityThemeCode']").children().each(function(){
																		if($(this).val()==activityThemeCode)
																			activityTheme = $(this).text();
																	});
							params.activityTheme = activityTheme;
							params.orgLevel = $("#activityCycleType").val();
							params.startDate = $("#startDate").val();
							params.endDate = $("#endDate").val();
							params.stateDesc = "创建";
							params.orgRange = $("#orgSelect").val();
							params.createUserName = "uni076";
							params.activityId = $("#activityId").val();
							params.parentId = $(":input[name='po.parentActivity']").val();
							params.userGroupId = $("#userGroupId").val();
							params.createOrgPath = "\root";
							...
							bpmServer.startFlow(loginId/*用户LoginId,必传*/,
									...
									function(json){
										...
										if(json.success == true){
											$(":input[name='processInstanceId']").val(json.processInstanceId);
											$(":input[name='activityProcInfo.processInstanceId']").val(json.processInstanceId);
											emptyExtral4Fro();	//客户经理扩展列表逻辑清除
											emptyExtral4FroPop();
											removeDisabled();
											if($(".inputUl").find(".checked").find("input[name='po.activityCycle']").val()=="3"){
											...
											//getSelectChannelId();//只保存选中的渠道;
											$.ligerui.loadingshow("正在保存...");
											$("#mainForm").submit();
										}else{
											$.ligerui.loadingshow("流程启动失败,请联系管理员进行解决");
											$(":button").attr("disabled",false);
										}
							});	
						}else{
...
if($("#successProductIds").val()=="" || $("#successProductIds").val()==null){
	...
	if($($("#match :radio")[1]).is(':checked')==true){
		$.ligerDialog.warn("当选择精确匹配时已选产品不能为空");
