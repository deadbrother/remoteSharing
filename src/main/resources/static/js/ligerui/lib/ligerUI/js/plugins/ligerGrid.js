...
//        pageStatMessage: '显示从{from}到{to}，总 {total} 条 。每页显示：{pagesize}',
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                <input type='button' class='l-bar-button  l-bar-btnfirst' value='首页'></input>");
            gridhtmlarr.push("                <input type='button' class='l-bar-button l-bar-btnprev' value='上一页'></input>");
            gridhtmlarr.push("                <span class='pcontrol'> </span>");
            gridhtmlarr.push("                <input type='button' class='l-bar-button  l-bar-btnnext' value='下一页'></input>");
            gridhtmlarr.push("                <input type='button' class='l-bar-button  l-bar-btnlast' value='尾页'></input>");
            gridhtmlarr.push("                <input type='button' class='l-bar-button  l-bar-btnload' value='刷新'></input>");
            gridhtmlarr.push("            </div>");
        ...
        */
        ...
        changePage: function (ctype,e)
        {
        	...
        	if(ctype=='button'){
        		...
        		btnPage = $("#"+element.id).text();
//        		btnPage = $("#"+e.toElement.id).text();
        	 ...
        	 if(ctype=='prev'||ctype=='first'){      		
             	if(p.page == 1){
                     return;
             	}
         	}
            if(ctype=='next'||ctype=='last'){      		
            ...
            switch (ctype){
            	case 'first': 
        	 		for(var i = 1;i<=p.pageCount;i++){
        	 			if(i>5)break;
        	 			$('.pcontrol').append('<input type="button" class="l-bar-button" id="page-button-'+i+'" value="'+i+'">');	
        	 		}
        	 		p.newPage = 1;
            	 	 break;
	            case 'prev': 
	            ...
	            case 'next': 
		            ...
		            if(pnum>p.pageCount){
		            	for(var i = p.pageCount-4;i<=p.pageCount;i++){
			            	if(i>(pnum))break;
			            	if(i<=0)break;
			      	 		$('.pcontrol').append('<button class="l-bar-button" id="page-button-'+i+'">'+i+'</button>');	
			      	 	}
		            }else{
		            	for(var i = p.newPage;i<=p.pageCount;i++){
			            	if(i>(pnum))break;
	            ...
	            case 'last': 
		            ...
		            $('.pcontrol').append('<button class="l-bar-button" id="page-button-'+p.pageCount+'">'+p.pageCount+'</button>');
		            break;
	            case 'button':
	            	var nv =  parseInt(btnPage);
	            	p.newPage = nv;
...
//                if (selectedIndex != -1) $('.l-bar-selectpagesize select', g.toolbar)[0].selectedIndex = selectedIndex;
            /*    if (p.switchPageSizeApplyComboBox && $.fn.ligerComboBox)
                ...
                $(".l-bar-selectpagesize input[name=rp]", g.toolbar).change(function(value){
